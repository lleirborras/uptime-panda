...
- ✅ URL forgery without the server secret (HMAC integrity)
- ✅ Monitor-id enumeration (signature defeats incremented IDs)

## What it does **not** defend

| Gap | Why it matters |
|-----|---------------|
| No `exp` claim | Token valid forever. URL leaks (browser history, logs, support screenshots) → permanent capability. |
...
get _signedScreenshotPath() {
    const now = Date.now();
    if (!this.__cachedScreenshotPath || now > this.__cacheExpiresAt) {
...
Option 3 if revocation becomes a customer-visible requirement.
