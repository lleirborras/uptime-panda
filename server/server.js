...
const resolveSocketHandlerFn = (mod) => {
    ...
    if (mod && typeof mod === "object") {
        const namedKey = Object.keys(mod).find((k) => typeof mod[k] === "function" && k.endsWith("SocketHandler"));
        if (namedKey) {
            return mod[namedKey];
        }
...
const socketHandlers = socketHandlersFs
    .readdirSync(socketHandlersDir)
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("monitorImportantHeartbeatListCount", async (monitorID, callback) => {
            try {
                ...
                if (monitorID == null) {
                    ...
                    const row = await getKnex()("heartbeat")
        ...
        socket.on("clearEvents", async (monitorID, callback) => {
            try {
                ...
                await getKnex()("heartbeat").where("monitor_id", monitorID).update({ msg: "", important: false });
...
async function checkOwner(userID, monitorID) {
    const row = await getKnex()("monitor").where({ id: monitorID, user_id: userID }).first("id");
...
async function startMonitor(userID, monitorID) {
    ...
    await getKnex()("monitor").where({ id: monitorID, user_id: userID }).update({ active: true });
...
async function pauseMonitor(userID, monitorID) {
    ...
    await getKnex()("monitor").where({ id: monitorID, user_id: userID }).update({ active: false });
