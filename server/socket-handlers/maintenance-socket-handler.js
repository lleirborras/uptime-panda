...
module.exports.maintenanceSocketHandler = (socket) => {
    ...
    onAuthed(
        socket,
        "addMaintenance",
        async (socket, maintenance, callback) => {
    ...
    onAuthed(
        ...
        async (socket, maintenanceID, monitors, callback) => {
    ...
    onAuthed(
        ...
        async (socket, maintenanceID, statusPages, callback) => {
            await requireOwnedMaintenance(maintenanceID, socket);
    ...
    onAuthed(
        socket,
        "getMaintenance",
        async (socket, maintenanceID, callback) => {
            log.debug("maintenance", `Get Maintenance: ${maintenanceID} User ID: ${socket.userID}`);

            let bean = await Maintenance.query().where({ id: maintenanceID, user_id: socket.userID }).first();
    ...
    onAuthed(
        ...
        async (socket, maintenanceID, callback) => {
            ...
            await getKnex()("maintenance").where({ id: maintenanceID, user_id: socket.userID }).delete();
