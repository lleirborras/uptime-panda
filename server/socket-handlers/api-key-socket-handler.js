...
module.exports.apiKeySocketHandler = (socket) => {
    ...
    onAuthed(
        ...
        async (socket, keyID, callback) => {
            ...
            // C-1
            await getKnex()("api_key").where({ id: keyID, user_id: socket.userID }).update({ active: false });
    ...
    onAuthed(
        ...
        async (socket, keyID, callback) => {
            ...
            // C-1
            await getKnex()("api_key").where({ id: keyID, user_id: socket.userID }).update({ active: true });
